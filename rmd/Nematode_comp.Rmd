...
ggplot(data=cbind(md$points, e), aes(x=MDS1, y=MDS2, fill=Habitat, colour=Habitat))+
  ...
  geom_label_repel(data=lab, aes(x=MDS1, y=MDS2, label=label), colour="black", 
             fill="white", size=4, label.padding = unit(0.2, "lines"))+
  scale_fill_viridis(discrete = TRUE)+
  scale_color_viridis(discrete = TRUE)+
  theme_bw() %+replace% large
```
...
can <- data.frame(wascores(md$points, b[, names(can)[1:5]]), Occurrence=can[1:5])
can$label <- gsub("_", " ", row.names(can))
slo <- data.frame(wascores(md$points, b[, names(slo)[1:5]]), Occurrence=slo[1:5])
slo$label <- gsub("_", " ", row.names(slo))
